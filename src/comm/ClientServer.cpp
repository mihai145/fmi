...
void FMI::Comm::ClientServer::send(channel_data buf,
    ...
    std::string file_name = comm_name + std::to_string(peer_id) + "_" +
                            std::to_string(dest) + "_" +
                            std::to_string(operation_num);
    operation_num++;
...
void FMI::Comm::ClientServer::recv(channel_data buf,
    ...
    std::string file_name = comm_name + std::to_string(dest) + "_" +
                            std::to_string(peer_id) + "_" +
                            std::to_string(operation_num);
    operation_num++;
...
void FMI::Comm::ClientServer::barrier() {
    ...
    std::string file_name =
        comm_name + std::to_string(peer_id) + barrier_suffix;
...
void FMI::Comm::ClientServer::reduce(channel_data sendbuf, channel_data recvbuf,
    ...
    if (peer_id == root) {
        ...
        std::memcpy(reinterpret_cast<void *>(recvbuf.buf), sendbuf.buf,
                    buffer_length);
        received[root] = true;
        applied[root] = true;
        unsigned int elapsed_time = 0;
        while ((max_timeout == 0 || elapsed_time < max_timeout) &&
               std::any_of(applied.begin(), applied.end(),
                           [](bool v) { return !v; })) {
            // Receive all values
            for (int i = 0; i < num_peers; i++) {
                ...
                std::string file_name =
                    comm_name + std::to_string(i) + "_reduce_" +
                    std::to_string(num_operations["reduce"]);
                if (download_object(
        ...
        std::string file_name = comm_name + std::to_string(peer_id) +
                                "_reduce_" +
                                std::to_string(num_operations["reduce"]);
        num_operations["reduce"]++;
...
double FMI::Comm::ClientServer::get_operation_latency(
    FMI::Utils::OperationInfo op_info) {
    ...
    case Utils::barrier: {
        ...
        double download = get_latency(
            1, num_peers, 1); // LIST used, modeled as download from one
    ...
    case Utils::scan:
        // Pattern is parallel (num_peers - 1, 1), (num_peers - 2, 1), ... ->
        // Slowest one is (num_peers - 1, 1)
        return get_latency(num_peers - 1, 1, size_in_bytes);
    }
    throw std::runtime_error("Operation not implemented");
}

double FMI::Comm::ClientServer::get_operation_price(
    FMI::Utils::OperationInfo op_info) {
    std::size_t size_in_bytes = op_info.data_size;
    switch (op_info.op) {
    case Utils::send:
